...
| R-002 | 上游（AKShare 等）可能回写历史；快照哈希只能固定本次输入 | P1 | 数据可信度阶段 | 依赖历史稳定性的结论 | 多次时间点重取同一区间的差异报告 + 差异触发 fail-closed 的测试 | Deferred（Owner Stage 待与 Gate F 数据有效性对齐，见 §5） |
...
- R-002 的 Owner Stage 仍写作“数据可信度阶段”。它与 R-003 属于同一类数据有效性问题，建议并入 Gate F / 数据有效性与时点正确性，并设 Blocks at = 依赖历史稳定性的 Gate F 结论。未获确认前保持原样。
