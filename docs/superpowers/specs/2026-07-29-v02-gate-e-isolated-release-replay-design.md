...
> 状态：用户于 2026-07-29 批准；独立设计门与可实现性复核通过；
> 尚未开始正式实施或双跑

## 1. 目标

使用 `release/v0.1-research/inputs/` 中经过发布清单和 SHA-256 验真的
10 标的冻结输入，在两个彼此隔离、禁止出站网络的全新 Python 3.11.15
环境中安装同一份 `a-share-quant 0.2.0` wheel，分别运行一次共享现金
Buy & Hold 组合。
...
- 28 个真实 no-bar 缺口、失败状态、实际权重和现金拖累没有被过滤、
  替换或美化。
...
2026-07-29 发现冻结目录存在一个未跟踪、被 `*.lock` 忽略的额外文件：
...
birth_and_modify_time
= 2026-07-29T20:40:01+08:00

recorded_at
= 2026-07-29T22:11:38+08:00
```

它不在 25 文件发布清单中，内容为空，不承载行情、公司行为、日历、
策略或参数。实施时必须：

1. 确认没有进程正在写该文件；
2. 将原文件原样移至
   `release/v0.2-gate-e/deviations/manifest.jsonl.lock.quarantined`；
3. 在 `outputs/Gate_E冻结输入偏差记录.md` 记录原路径、大小、哈希、
   时间、新路径、移动原因和研究语义判断；
4. 移动后验证冻结输入目录恰好是清单声明的 25 个文件；
5. 验收前后重新验证全部 25 个文件字节未变。

该移动必须由已验证父目录描述符上的系统级排他原子重命名一次完成，
不得拆成“创建硬链接后再删除原名”，也不得回退到可覆盖目标的普通
重命名。目标已存在、目录绑定变化或平台缺少 no-replace 原语时均
保守失败。
...
- `uv.lock` SHA-256；
- Python 3.11.15 基础解释器路径和 SHA-256；
- wheelhouse 每个文件的路径、版本、大小和 SHA-256；
...
```text
600030: 17 个官方 session，最长连续 10 个
600900: 11 个官方 session，最长连续 10 个
其他 8 个标的: 0
合计: 28
```
